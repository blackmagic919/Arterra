...
public class TerrainGenerator : ScriptableObject
{
    ...
    public ComputeShader surfaceTranscriber;
    public ComputeShader mapSimplifier;
    public ComputeShader expFalloffNoise;
    ...
    public void ApplyMapFalloff(ComputeBuffer noiseMap, int chunkSize, int meshSkipInc, float falloffIntensity){
        int numPointsPerAxis = chunkSize / meshSkipInc + 1;
        expFalloffNoise.SetBuffer(0, "noiseMap", noiseMap);
        expFalloffNoise.SetFloat("falloffIntensity", falloffIntensity);
        expFalloffNoise.SetInt("numPointsPerAxis", numPointsPerAxis);

        expFalloffNoise.GetKernelThreadGroupSizes(0, out uint threadGroupSize, out _, out _);
        int numThreadsPerAxis = Mathf.CeilToInt(numPointsPerAxis / (float)threadGroupSize);
        expFalloffNoise.Dispatch(0, numThreadsPerAxis, numThreadsPerAxis, 1);
    }
    ...
    public ComputeBuffer CombineTerrainMapsGPU(ComputeBuffer count, ComputeBuffer contBuffer, ComputeBuffer erosionBuffer, ComputeBuffer PVBuffer, int maxPoints, float terrainOffset, Queue<ComputeBuffer> bufferHandle)
    {
    ...
    public void TranscribeSurfaceMap(ComputeBuffer memory, ComputeBuffer addresses, int addressIndex, ComputeBuffer surfaceMap, int numPoints, bool isFloat){
        if(isFloat)
            this.surfaceTranscriber.DisableKeyword("PROCESS_INT");
        else
            this.surfaceTranscriber.EnableKeyword("PROCESS_INT");

        this.surfaceTranscriber.SetBuffer(0, "SurfaceMap", surfaceMap);
        this.surfaceTranscriber.SetInt("numSurfacePoints", numPoints);

        this.surfaceTranscriber.SetBuffer(0, "_MemoryBuffer", memory);
        this.surfaceTranscriber.SetBuffer(0, "_AddressDict", addresses);
        this.surfaceTranscriber.SetInt("addressIndex", addressIndex);

        this.surfaceTranscriber.GetKernelThreadGroupSizes(0, out uint threadGroupSize, out _, out _);
        int numThreadsPerAxis = Mathf.CeilToInt(numPoints / (float)threadGroupSize);
        this.surfaceTranscriber.Dispatch(0, numThreadsPerAxis, 1, 1);
    }

    public ComputeBuffer SimplifyMap(ComputeBuffer memory, ComputeBuffer addresses, int addressIndex, int chunkSize, int sourceSkipInc, int destSkipInc, bool isFloat, Queue<ComputeBuffer> bufferHandle = null)
    {
    ...
    public ComputeBuffer AnalyzeNoiseMapGPU(ComputeBuffer checks, ComputeBuffer count, NoiseData noiseData, Vector3 offset, float maxInfluenceHeight, int chunkSize, int maxPoints, bool centerNoise, Queue<ComputeBuffer> bufferHandle)
    {
    ...
    public ComputeBuffer AnalyzeRawNoiseMap(ComputeBuffer points, ComputeBuffer count, NoiseData noiseData, Vector3 offset, int chunkSize, int maxPoints, Queue<ComputeBuffer> bufferHandle)
    {
        ComputeBuffer result = new ComputeBuffer(maxPoints, sizeof(float), ComputeBufferType.Structured);
        ComputeBuffer args = UtilityBuffers.CountToArgs(rawNoiseSampler, count);
    ...
    public ComputeBuffer AnalyzeBiome(SurfaceChunk.NoiseMaps noiseMaps, ComputeBuffer count, int maxPoints, Queue<ComputeBuffer> bufferHandle)
    {
