...
public unsafe struct Entity{
    /// <summary>
    /// A virtual function pointer that points to the entity's update function defined by the entity's type.
    /// Use the update function to perform computations related to the core logic of the entity such
    /// as pathfinding or updating its state machine. This function is called from an unsafe context and
    /// is not permitted to access managed objects.
    /// </summary>
    public FunctionPointer<IEntity.UpdateDelegate> _Update;
    /// <summary>
    /// A virtual function pointer that points to the entity's disable function defined by the entity's type.
    /// An entity should use the disable function to release any information not exposed to the <see cref="EntityController"/>
    /// and mark it as disabled. Then the <see cref="EntityController"/> will release the entity's resources and remove it from the system.
    /// This is necessary to ensure that both the entity and its controller are not still accessing the entity's resources when it is released.
    /// </summary>
...
public interface IEntity{
    ...
    /// <summary>
    /// The function signature all update functions must follow in the <see cref="Entity"> unmanaged interface </see>.
    /// The update function itself is provided information about its instance, and a reference context and must determine
    /// its behavior off this information. <see cref="Entity._Update"/> for more info.
    /// </summary>
    /// <param name="entity">A pointer to the entity that is the entity's instance.</param>
    /// <param name="context">A shared context to obtain world setttings and data. See <see cref="EntityJob.Context"/> for more info. </param>
    //public abstract unsafe IntPtr Deserialize(ref Entity entity, out int3 GCoord);
    public unsafe delegate void UpdateDelegate(Entity* entity, EntityJob.Context* context);
    /// <summary>
    /// The function signature all disable functions must follow in the <see cref="Entity"> unmanaged interface </see>.
    /// <see cref="Entity._Disable"/> for more info.
    /// </summary>
