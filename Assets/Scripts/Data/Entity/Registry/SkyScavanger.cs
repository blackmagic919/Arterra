...
[CreateAssetMenu(menuName = "Entity/SkyScavanger")]
public class SkyScavanger : Authoring
{
    ...
    {  
        ...
        private unsafe bool FindPreyLanding(){
            if(!settings.recognition.FindPreferredPrey(this, out Entity prey, CanEatEntity)){
                RandomFly();
                return false;
            }
        ...
        private static unsafe void FindPrey(Animal self){
            ...
            if(!self.settings.recognition.FindPreferredPrey(self, out Entity prey, CanEatEntity)){
                self.RandomFly();
                self.TaskIndex = 2;
                return;   
            }
