...
public class SurfaceCarnivore : Authoring
{
    ...
    {  
        ...
        private unsafe void DetectPredator(){
            ...
            byte* path = PathFinder.FindPathAlongRay(GCoord, ref rayDir, PathDist + 1, info.profile, EntityJob.cxt, out int pLen);
        ...
        private static unsafe void FindPrey(Animal self){
            ...
            int PathDist = self.settings.movement.pathDistance;
            int3 destination = (int3)prey.position - self.GCoord;
            byte* path = PathFinder.FindPath(self.GCoord, destination, PathDist + 1, self.info.profile, EntityJob.cxt, out int pLen);
            self.pathFinder = new PathFinder.PathInfo(self.GCoord, path, pLen);
            self.TaskIndex = 4;
        }
        ...
        private static unsafe void ChasePrey(Animal self){
            ...
            Movement.FollowDynamicPath(self.info.profile, ref self.pathFinder, ref self.tCollider, prey.position,
        ...
        private static unsafe void ChaseMate(Animal self){//I feel you man
            ...
            Movement.FollowDynamicPath(self.info.profile, ref self.pathFinder, ref self.tCollider, mate.position,
            self.settings.movement.walkSpeed, self.settings.movement.rotSpeed, self.settings.movement.acceleration);
        ...
        private static unsafe void RunFromPredator(Animal self){
            Movement.FollowStaticPath(self.info.profile, ref self.pathFinder, ref self.tCollider, self.settings.movement.runSpeed, 
