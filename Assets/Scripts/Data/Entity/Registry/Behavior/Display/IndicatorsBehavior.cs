...
public static class Indicators {
    ...
    public static void PlayWaterSplash(Entity entity, float weight = 1) {
        ...
        strength *= weight / 1500; //arbitrary rescaling
        strength = math.clamp(strength, 0, 1 - 1e3f);

        FMOD.Studio.EventInstance evnt = AudioManager.CreateEvent(AudioEvents.Action_WaterSplash, entity.position);
        evnt.setParameterByName("Splash Strength", (float)strength);
    }

    public static void PlayDamageTick(Entity entity, float healthPercent) {
        FMOD.Studio.EventInstance evnt = AudioManager.CreateEvent(AudioEvents.Action_Damage, entity.position);
        evnt.setParameterByName("Damage", healthPercent);
    }
...
public class InidcatorsBehavior : SpeciesBehavior {
    ...
    private void OnDamaged(object self, object attacker, object cxt) {
        if (cxt is not RefTuple<(float damage, float3 kb)> d1) return;
        float thresh = vit != null ? vit.stats.MaxHealth * 0.01f : 0.1f;
        if (d1.Value.damage < thresh) return;
        if (self is not Entity selfE) return;
        
        selfE.eventCtrl.RaiseEvent(GameEvent.Entity_DamageIndication, self, attacker, cxt);
        if (cxt is not RefTuple<(float damage, float3 kb)> data) return;

        EntityManager.AddHandlerEvent(() => {
            Indicators.DisplayDamageParticle(selfE.position, data.Value.kb); 
            Indicators.PlayDamageTick(selfE, data.Value.damage/vit.stats.MaxHealth); 
        });
    }
