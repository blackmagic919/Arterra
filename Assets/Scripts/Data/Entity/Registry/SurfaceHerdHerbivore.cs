...
[CreateAssetMenu(menuName = "Generation/Entity/SurfaceHerdHerbivore")]
public class SurfaceHerdHerbivore : Authoring
{
    ...
    public class Animal : Entity, IAttackable, IMateable, Movement.IBoid {
        ...
        public void TakeDamage(float damage, float3 knockback, Entity attacker) {
            ...
            if (recog.IsPredator) TaskIndex = 8u; //if predator run away
            else if (recog.IsMate) TaskIndex = 9u; //if mate fight back
            else if (recog.IsPrey) TaskIndex = 9u; //if prey fight back
            else TaskIndex = settings.Recognition.FightAggressor ? 9u : 8u; //if unknown, depends
            if (TaskIndex == 9 && attacker is not IAttackable) TaskIndex = 8u;  //Don't try to attack a non-attackable entity
        ...
        public override void Update() {
            ...
            if (TaskIndex != 12 && vitality.IsDead) {
        ...
        private class AnimalController {
            ...
            private static readonly string[] AnimationNames = new string[]{
                "IsIdling", "IsWalking", null, "IsRunning", "IsEating",
