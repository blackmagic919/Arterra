...
public class PlayerCollider {
    ...
    public float2 BilinearDisplacement(float2 posGS, Func<int2, int> SampleTerrain) {
        int x0 = (int)Math.Floor(posGS.x); int x1 = x0 + 1;
        int y0 = (int)Math.Floor(posGS.y); int y1 = y0 + 1;
    ...
    public float BilinearGradientLength(float density, float2 pos, float2 normal, Func<int2, int> SampleTerrain) {
