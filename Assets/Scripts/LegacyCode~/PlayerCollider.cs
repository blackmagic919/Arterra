...
namespace Arterra.Configuration.Gameplay.Player{
    /// <summary> The player's terrain collider; identical to <see cref="TerrainCollider"/>
    /// except executable both in fixed update and in a Unity Job. </summary>
    public class PlayerCollider {
        private float IsoValue => CPUMapManager.IsoValue;
        /// <summary> The optional callback that will be called when the entity collides
        /// with the terrain. Must be rebound on deserialization.  </summary>
        [JsonIgnore]
        public Action<float> OnHitGround;
        /// <summary> The <see cref="TerrainCollider.Transform">Transform</see> information about the entity. </summary>
        public TerrainCollider.Transform transform;
        /// <summary> Whether or not gravity applies to the entity. </summary>
        /// <remarks> If gravity is not applied, entity will experience 3D friction </remarks> 
        public bool useGravity;

        private float3 TrilinearDisplacement(float3 posGS) {
            //Calculate Density
        ...
        private float2 BilinearDisplacement(float2 posGS, Func<int2, int> SampleTerrain) {
            int x0 = (int)Math.Floor(posGS.x); int x1 = x0 + 1;
            int y0 = (int)Math.Floor(posGS.y); int y1 = y0 + 1;

            int c00 = SampleTerrain(new int2(x0, y0));
            int c10 = SampleTerrain(new int2(x1, y0));
            int c01 = SampleTerrain(new int2(x0, y1));
            int c11 = SampleTerrain(new int2(x1, y1));
        ...
        private float LinearDisplacement(float t, Func<int, int> SampleTerrain) {
        ...
        private float LinearGradientLength(float density, float pos, float normal, Func<int, int> SampleTerrain) {
        ...
        private float BilinearGradientLength(float density, float2 pos, float2 normal, Func<int2, int> SampleTerrain) {
            ...
            if (tMax.y >= tMax.x) { eDen = LinearDensity(pos.y, (int y) => SampleTerrain(new int2(Mathf.RoundToInt(pos.x), y))); } 
            else { eDen = LinearDensity(pos.x, (int x) => SampleTerrain(new int2(x, Mathf.RoundToInt(pos.y)))); }
            if (eDen >= density) return 0;

            return math.clamp((IsoValue - density) / (eDen - density), 0, 1) * t;
        }

        private float TrilinearGradientLength(float density, float3 pos, float3 normal) {
            ...
            if (t == tMax.x) { fDen = BilinearDensity(pos.y, pos.z, (int y, int z) => SampleTerrain(new int3(Mathf.RoundToInt(pos.x), y, z))); } else if (t == tMax.y) { fDen = BilinearDensity(pos.x, pos.z, (int x, int z) => SampleTerrain(new int3(x, Mathf.RoundToInt(pos.y), z))); } else { fDen = BilinearDensity(pos.x, pos.y, (int x, int y) => SampleTerrain(new int3(x, y, Mathf.RoundToInt(pos.z)))); }
        ...
        */
        ...
        private bool SampleEdgeCollision(float3 originGS, float3 boundsGS, out float3 displacement) {
            ...
            for (int x = minC.x; x <= maxC.x; x++) {
                for (int i = 0; i < 4; i++) {
                    ...
                    dis = BilinearDisplacement(corner, c => SampleTerrain(new int3(x, c.x, c.y)));
            ...
            for (int y = minC.y; y <= maxC.y; y++) {
                for (int i = 0; i < 4; i++) {
                    ...
                    dis = BilinearDisplacement(corner, c => SampleTerrain(new int3(c.x, y, c.y)));
            ...
            for (int z = minC.z; z <= maxC.z; z++) {
                for (int i = 0; i < 4; i++) {
                    ...
                    dis = BilinearDisplacement(corner, c => SampleTerrain(new int3(c.x, c.y, z)));
        ...
        /// <summary> Tests whether a given bounds intersects with the terrain. </summary>
        /// <param name="originGS">The origin in grid space of the bounds</param>
        /// <param name="boundsGS">The size in grid space from the origin of the bounds</param>
        /// <param name="displacement">If intersecting the terrain, the shortest vector to resolve the collision</param>
        /// <returns>Whether or not the bounds intersects the terrain</returns>
        ...
        /// <summary> Initializes the player collider  </summary>
        /// <param name="settings">The settings for the player collider</param>
        /// <param name="position">The position in grid space of the origin of the collider.</param>
