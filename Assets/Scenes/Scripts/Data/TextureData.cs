...
public class TextureData : UpdatableData
{
    ...
    /*public void ApplyToMaterial(Material mat, List<GenerationHeightData.BMaterial> materials)
    {
        ...
        baseColors = new ComputeBuffer(materials.Count, sizeof(float) * 4);
        baseColors.SetData(materials.Select(x => x.materialData.color).ToArray());

        baseColorStrengths = new ComputeBuffer(materials.Count, sizeof(float));
        baseColorStrengths.SetData(materials.Select(x => x.materialData.baseColorStrength).ToArray());

        baseTextureScales = new ComputeBuffer(materials.Count, sizeof(float));
        baseTextureScales.SetData(materials.Select(x => x.materialData.textureScale).ToArray());

        baseTextureIndexes = new ComputeBuffer(materials.Count, sizeof(float));
        baseTextureIndexes.SetData(materials.Select(x => (float)x.materialData.textureInd).ToArray());

        //mat.SetMatrix("_LocalToWorld", _LocalToWorld);
        mat.SetBuffer("baseColors", baseColors);
        mat.SetBuffer("baseColorStrength", baseColorStrengths);
        mat.SetBuffer("baseTextureScales", baseTextureScales);
        mat.SetBuffer("baseTextureIndexes", baseTextureIndexes);
    }*/
